...
$jobs += Start-ThreadJob {   
    ...
    $using:files | % {
        $o = $_-replace"\.c$",".o"
        $z += $o
        & $using:CC $_ -c -o $o -O3 -g -DNDEBUG -D_CRT_SECURE_NO_WARNINGS -D_CRT_NONSTDC_NO_DEPRECATE $using:FLAGS $using:rlsFF || Write-Host "Error in compilation"
    }
    & $using:CC $z asm.o -g -o a.exe -O3 "-Wl,/subsystem:console" "-Wl,/MAP:release.map" $using:FLAGS $using:LF $using:rlsLF $using:rlsFF || Write-Host "Error in compilation"
}
$jobs += Start-ThreadJob {
